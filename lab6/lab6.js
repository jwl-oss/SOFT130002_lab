...
function testMail(telephone,mail) {
    let telephone_test = /^1\d{10}$/;//1开头，后面任意十位数字；
    let mail_test = /^[a-zA-Z0-9_-]+@[a-zA-Z0-9_-]+(\.[a-zA-Z0-9_-]+)+$/;//只允许英文字母，数字，下划线和“.”组成的邮箱
    let telephone_judge =telephone_test.test(telephone);
    let mail_judge = mail_test.test(mail);
    function output() {
...
testMail(18051150314,"19302010076@fudan.edu.cn");
...
function testRedundancy(str) {
    const test = /\b([a-z]+) \1\b/ig ;
    let array1 = str.split(" ");
    let array2 = [];
    let array = [];
    let n = 0;
    for (let i = 0;i < array1.length-1;i++){
        array2[i] = array1[i]+" "+array1[i+1];//将临近的两个字母放入。
        if (array2[i].match(test)){
            array[n] = array2[i];
            n++;
        }
    }
    if (array.length > 10){//数组中元素超过十个；
        array.sort(function (s,t){//排序；
            let a = s.toLowerCase();
            let b = t.toLowerCase();
            if (a < b){
                return -1;
        ...
        array.splice(10,array.length-10);//对数组进行添加或删除元素。
    }
...
function testKeyBoard(wantInput, actualInput) {
    ...
    for(let i=0,j=0;i<wantInput.length;){
        if(wantInput[i].toLowerCase()===actualInput[j].toLowerCase())//不分大小写判断；
        {
            set.add(wantInput[i]);
            i++;
        }
        else{
            if(j===actualInput.length-1)//actualInput比wantInput短；
                i++;
            else {
                i++;
                j++;
...
function testSpecialReverse(str) {
    ...
    console.log(result.join());//将元素组成字符串；
}
...
lengthOfLongestSubstring("abcddd");
/*
8.
背景：
